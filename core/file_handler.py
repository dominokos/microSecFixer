...
def is_repo_downloaded(repo_folder: str) -> bool:
...
def detection_comment(file_name: str, line: str):
...
def detection_import(line: str):
    """Checks if provided line is an import statement.
    """

    try:
        return line.split()[0] == "import"
    except:
        return False


def detection_config(file_name: str):
...
def search_keywords(keywords: str):
    ...
    if not is_repo_downloaded(repo_folder):
...
def file_as_lines(raw_file: str):
...
def detect_microservice(file_path: str, dfd):
    """Finds microservice that a file belongs to.
    """

    #XXXTHIS IS COMPLETE NONSENSE JUST TO GET A COMMIT DONE SO I CAN CONTINUE WORK ON ANOTHER MACHINE
    microservices_set = {x: dfd["elements"][x] for x in dfd["elements"].keys() if dfd["elements"][x]["type"] == "Service"}
...
def resolve_url(url: str, microservice: str, dfd) -> str:
    ...
    #THIS IS COMPLETE NONSENSE JUST TO GET A COMMIT DONE SO I CAN CONTINUE WORK ON ANOTHER MACHINE
    microservices = {x: dfd["elements"][x] for x in dfd["elements"].keys() if dfd["elements"][x]["type"] == "Service"}
...
def get_repo_contents_local(repo_path: str, path: str) -> set:
    ...
    if not is_repo_downloaded(local_repo_path):
        download_repo(repo_path)
